...
def mark_visited(current_map, section_top_left):
    print(f"Visiting {SELECTION_SIZE}x{SELECTION_SIZE} square with top left: {section_top_left}")
...
def repeat_found(current_map):
    ...
    for y, row in enumerate(current_map):
        if row[start_index - 1] or row[end_index + 1]:
            all_visited = True
    ...
    for x in range(start_index, end_index):
        if current_map[start_index - 1][x] or current_map[end_index - 1][x]:
            all_visited = True
...
attempts = 0
while not repeat_found(full_map):
    attempts += 1
    print('Attempt', attempts)
    random_selection_foo = select_random_section()
    # Print the map with the new value
    print('Map with last visit:')
    mark_visited(full_map, random_selection_foo)
    pretty_print_map_with_selection(full_map, random_selection_foo)
